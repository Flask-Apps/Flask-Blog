...
class APITestCase(unittest.TestCase):
    ...
    def get_api_headers(self, username, password):
        """
        Return common headers that need to be sent with most
        API requests including the authentication credentials and
        the MIME type-related headers
        """
        return {
            "Authorization": "Basic "
    ...
    def test_no_auth(self):
        # request wit no authentication credentials is rejected
    ...
    def test_bad_auth(self):
        ...
        u = User(email="joe@example.com", password="cat", confirmed=True, role=r)
        db.session.add(u)
        db.session.commit()

        # authenticate with bad password
        response = self.client.get(
            "/api/v1/posts/", headers=self.get_api_headers("joe@example.com", "dog")
        )
        self.assertEqual(response.status_code, 401)
    ...
    def test_token_auth(self):
        ...
        response = self.client.post(
            "/api/v1/tokens/", headers=self.get_api_headers("joe@example.com", "cat")
        )
    ...
    def test_unconfirmed_account(self):
        ...
        r = Role.query.filter_by(name="User").first()
        self.assertIsNotNone(r)
        u = User(email="joe@example.com", password="cat", confirmed=False, role=r)
        db.session.add(u)
        db.session.commit()

        # get list of posts with the unconfirmed account
        response = self.client.get(
            "/api/v1/posts/", headers=self.get_api_headers("joe@example.com", "cat")
        )
    ...
    def test_posts(self):
        ...
        response = self.client.post(
            "/api/v1/posts/",
            headers=self.get_api_headers("joe@example.com", "cat"),
            # flask test client doesnot automatically encode to JSON
        ...
        # print(url)
        response = self.client.get(
            url, headers=self.get_api_headers("joe@example.com", "cat")
        )
        self.assertEqual(response.status_code, 200)
        json_response = json.loads(response.get_data(as_text=True))
        # print(json_response)
    ...
    def test_users(self):
        ...
        response = self.client.get(
            ...
            headers=self.get_api_headers("sam@example.com", "dog"),
        )
        self.assertEqual(response.status_code, 200)
        json_response = json.loads(response.get_data(as_text=True))
        self.assertEqual(json_response["username"], "joe")
        response = self.client.get(
